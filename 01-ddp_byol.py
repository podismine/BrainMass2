import torch
import yaml
from network_dataset import Pretrain_data as Task1Data
import os
import time
...
def main():
    ...
    for epoch_counter in range(config['trainer']['max_epochs']):
        ...
        for step, (batch_view_1, batch_view_2, age, sex) in enumerate(train_loader):
            B = len(batch_view_1)
            ...
            sex = sex.to(device,non_blocking=True).float()

            loss_byol, acc, nce, mse = model(batch_view_1, batch_view_2, age, sex)
